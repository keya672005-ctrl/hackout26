...
2. **Pull state from these sources (in order):**
   - Plan files referenced this session (check `C:\Users\singh\.claude\plans\` if a plan was mentioned).
   ...
   - Memory files written or updated (`C:\Users\singh\.claude\projects\<project>\memory\`).
...
5. **No emojis, no hype, no "great job" summaries.** Terse and concrete — paths, commands, decisions. Match the tone of a seasoned engineer handing off at end-of-shift.
