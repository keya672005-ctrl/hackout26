...
- **Frontend:** `src/api/client.js` is the fetch client (same-origin `/api` paths). `vite.config.js` proxies `/api` → `127.0.0.1:8000`, so no environment-specific base URL is needed. `src/index.css` carries the design tokens so Phase 4 doesn't re-derive a palette. *(The **colours** are still these. The **type** is not: Sora / IBM Plex were replaced after the mid-eval by a single Times New Roman family -- see CONTEXT.md section Design. A moss/bone repaint went with that pass and was reverted the same evening. Both swaps touched one stylesheet and two colour literals rather than every component, because the token names never changed -- which is the whole return on defining them in Phase 0.)* `src/App.jsx` is a stub screen that calls `/api/ping` and shows the live API status.
...
**Verification checklist:** *(128 automated checks pass locally -- run `node verify_phase6.mjs` in `frontend/`; add `--target <url>` to run the same checks against a running deployment, where 125 of them apply -- the build-time checks have nothing to read)*
- [x] Full user journey (site list → site detail → verification status → export) works **on the production artifact** -- 116/116 against the Docker image itself, driven by *clicking* (card → detail → report link → export → breadcrumb → brand), not by typing URLs, because the checklist item is that the links work. The export step is rendered through Chrome's real print pipeline and the PDF is checked for A4 bytes, so "export" means the artifact, not the button. **Confirmed on the live Render URL** at https://algae-carbon-platform.onrender.com -- the same journey, clicked through the deployed service over the public internet: **117/117** via `node verify_phase6.mjs --target https://algae-carbon-platform.onrender.com`. Re-run and still 117/117 after the typography restyle shipped, against the deployed build rather than a local one. **Re-verified at 125/125 after the dark-theme pass was deployed** (`016b46c`) -- the count rose from 117 because the theme work added checks of its own, and the deployed JS, CSS and favicon were additionally compared to `frontend/dist` by SHA-256 and match byte for byte, which is the only way to be sure *which* build is answering.
...
### Post-mid-eval — six pond blocks and a facility filter
Two site cards was too thin a portfolio to show what the platform is for, so the two facility-wide footprints were subdivided into **six measured raceway blocks** (Earthrise x3, Cyanotech x3) and the overview grew a facility selector.

- **The blocks were delineated from the imagery, not drawn by hand.** The per-pixel temporal-median NDCI over each facility footprint was thresholded and connected-component labelled; a block is a contiguous run of pixels that holds chlorophyll across *every* date in the window. The resulting rectangles line up with the raceway channels visible in the scene, and the six blocks sum to roughly the 20 ha of pond the single-AOI Phase 1 pass had measured -- the cross-check that the subdivision did not invent area.
- **Every block sits inside the footprint Phase 1 validated.** That containment is load-bearing: clustering over the wider property also finds the Imperial Valley cropland south of Earthrise and the coastal strip west of Cyanotech, both chlorophyll-rich and neither a pond. NDCI cannot tell a raceway from an alfalfa field, which is exactly why E.I.D. Parry was cut in Phase 1.
- **The mask cutoff moved from the 60th percentile to the 40th, and the reason is not the verdicts.** A cutoff tuned for a footprint that is mostly *not* pond throws away real pond pixels when the footprint is one block. At p40 the measured masks land close to the delineated block areas (1.7 ha against 1.8 documented, where p60 gave 1.1) -- the mask is measuring the block rather than its core.
- **`GET /api/sites` now carries `operator` and `location`**, so the overview can group and filter without a second round-trip. Additive to the contract in architecture.md.
- **The filter is gated, not just shipped:** the Phase 4 gate drives the real `<select>` the way a reader does (native setter plus a dispatched change event, so React's listener fires) and asserts the grid narrows to exactly the blocks the API attributes to that operator, then restores. A filter that silently dropped or duplicated a block fails there.
- **Two gates had to be told what "the demo contrast" means.** Phases 4 and 5 took `sites[0]` and `sites[1]` and assumed those were the verified and the flagged one. With six blocks they are both verified, so the pair is now chosen *by verdict*. The Phase 3 CO2 snapshot was also re-pinned: the figures fell because each site is now one block rather than a whole facility.
- **Verdicts are 3 verified / 3 flagged** -- Earthrise reconciles, Cyanotech does not. Only one of the three flagged blocks (`site-b`) carries the deliberate 34% reporting drift; the other two flag on their own, because Cyanotech's blocks have 6 cloud-free dates against Earthrise's 9 and a 15-day revisit gap, so the rolling comparison has less to work with. That is a real property of the data rather than something injected, and DEMO.md says so in the words to use on stage.

### Post-mid-eval — a dark theme, a light fallback, and motion
The interface was restyled for the judging table: other teams had visibly invested in UI, and a single flat white page in Times was losing a comparison it did not need to lose. The palette question had been opened once before and closed badly (an editorial moss/bone repaint, rejected on sight), so this pass was scoped and agreed up front rather than discovered.

- **Two themes off one token set, dark by default.** Deep ocean ground, a slow four-wash aurora, glass panels translucent to it, teal that glows. (The first pass used three washes at roughly a third opacity and read as a faint smudge in one corner on a projector; the washes and `--surface`'s alpha were raised together in `016b46c`, and they have to keep moving together -- the stronger the ground, the more of it bleeds through a translucent panel, and body copy still has to be readable over whichever wash sits behind that card.) `:root[data-theme='light']` restores the **Phase 0 canvas palette unchanged** -- the rejected repaint is why the light theme is the canvas and not a third invention. Every rule in `index.css` reads tokens only, which is what made the second theme a dozen declarations rather than a parallel stylesheet.
- **The toggle is a projector-failure control, not a flourish.** A washed-out projector flattens darks into one grey; one click switches to the palette that survives it. `data-theme` is set on `<html>` by an inline script in `index.html` **before first paint**, because React mounts too late to prevent a flash of the wrong theme, and the choice is kept in `localStorage` behind a try/catch (a private window must degrade to "does not persist", not to a crash).
- **Typography split rather than replaced.** Times keeps the display voice -- page titles, figure titles, site names, the headline figures -- and the system sans takes labels, badges, controls and body copy, with a real mono for report ids and reporting periods. Body drops 17px to 15.5px because the sans sets larger at the same size. No webfonts: `scripts/demo.js` fails the build if the shell or its assets name a single external origin, and that check is what makes the offline stage fallback's promise real.
- **Motion is layered over a resting state that is already correct.** Staggered card entrances, a left-to-right plot wipe, self-drawing sparklines (`pathLength={1}`, so one keyframe fits any data), a growing divergence gauge, a pulsing halo on the review badge. Every from-state lives in a keyframe and never in the element's own rule, so `prefers-reduced-motion` and `@media print` disable motion by disabling animation and every element is simply *there*. The inverse -- an entrance that hides its own content when switched off -- is how a report exports blank.
- **The figures are never animated, and that is a gate constraint, not taste.** Phases 4, 5 and 6 read `.tile-value`, `.hero-value` and `.site-figure-value` `textContent` and compare it to the API response behind it. A count-up would read "0" mid-flight and fail a 90-second gate. Depth and motion everywhere else; the numbers are correct from frame one.
- **Print needed hardening for a reason specific to theming.** The themes are attribute-qualified (`:root[data-theme='dark']`, 0-1-1) and the print block's bare `:root` is 0-0-1 -- so the dark theme outranked it and paper would have exported black. The print block now names all three selectors, and kills animation, `backdrop-filter` and every glow globally.
- **Verified against all six gates and the fallback, not by eye alone:** 38/38, 47/47, 34/34, 120/120, 184/184, 128/128, and `npm run demo:check` clean including "the page loads nothing from the internet". The theme switch was separately driven in a real browser to prove the attribute, the stored choice, the button's label and the reload all round-trip with no console errors. **And confirmed on the deployed service, not just locally:** 125/125 via `node verify_phase6.mjs --target https://algae-carbon-platform.onrender.com`, with `[data-theme=dark]`, `[data-theme=light]`, the aurora, `backdrop-filter`, `prefers-reduced-motion` and the `@media print` block all present in the minified CSS the server actually sends -- and zero external origins in it.

### Post-mid-eval — an operator's dashboard: scope and a watchlist
A late request, and the right one: the overview listed six blocks the same way for everyone, but an operator does not run six ponds — they run the ones they are responsible for. Two changes, one screen, no backend work.

- **The facility selector became a scope selector,** with `<optgroup>`s for region and for facility. Region is derived from each site's own `location` (the second-to-last segment — town, district, **state**, country) rather than stored as a new field, so it needed no API change and a seventh block groups itself. **Worth saying out loud:** in the current data the two axes produce identical groupings, because each facility happens to sit in a region of its own. That is a property of having two facilities, not of the model, and a judge who tries both will notice — so the code says so in a comment rather than pretending the coincidence is a feature.
- **A watchlist:** a `Watch` checkbox per block, a `Showing: All blocks / My watchlist` toggle, and a summary strip that totals *exactly* the watched blocks — CO2 fixed, and the verified/flagged split of it. The split is the point: a portfolio total is worth what the imagery supports, which is the platform's whole argument applied to the reader's own selection.
- **The selection is per-browser (`localStorage`), and the empty state says so** — "there are no accounts in this build". There is no auth, so there is no operator for a backend to store this against; inventing a user menu would imply one. Reads and writes are wrapped because a private window throws on access, and the dashboard has to degrade to "does not persist", not to a blank screen.
- **The watch control is a *sibling* of the card, never a child.** `.site-card` is an anchor and the gates click it and read its `href` to prove the grid navigates; a checkbox nested inside an anchor fights it for the click, and "select a block" and "open a block" would become the same gesture. A `.site-cell` wrapper keeps them apart for the reader and leaves the anchor exactly as the gates expect.
- **Caught by running it:** the reporting-window control is also a `.segmented` and also has an "All" button, and it sits earlier in the DOM — so selecting by button text clicked the wrong group and silently changed the window instead of the mode. The gate now selects by `data-mode`, and the reader-facing label became "All blocks", because two identical labels a few centimetres apart is a coin toss for a human too.
- **Caught by the gate:** the Phase 4 "no hard-coded frontend mocks" check failed on `src/lib/scope.js` — a doc comment quoting a seeded location verbatim. The comment was reworded rather than the check loosened. A gate that greps source cannot tell an example in prose from a mock in code, and under time pressure the temptation is to weaken it; the inverse of the Phase 6 lesson, where a check passed on its own comment.
- **Gated, not just shipped:** Phase 4 is **120/120** (was 107). The new checks drive the real `<select>` and the real checkboxes the way a reader does, assert the grid narrows to exactly the blocks the API attributes to that region or operator, assert the summary totals match the API figures for the ticked blocks, and **reload the page** to prove the selection survives — a watchlist that forgets itself on refresh is not a watchlist.
...
- [x] Slide deck built, in two forms off one set of figures. `BioFix_Deck.pptx` is a real PowerPoint -- native text frames and freeform vector paths, no slide is a screenshot -- regenerated by `python scripts/make_deck.py` (needs `python-pptx`); the same ten slides also exist as a published HTML deck that runs from a browser tab with arrow-key navigation and its own projector-safe light toggle. **Every figure on every slide was pulled from the deployed API**, not retyped: the two reconciliation charts plot the real 28-day series, and the portfolio, report id and agreement figures match what the demo shows on stage. Rendered through PowerPoint itself to check, which caught a footer rule striking through the caption on all ten slides.
...
- **The fallback is the deployed shape, not a local approximation of it.** `scripts/demo.js` starts the same uvicorn process with `STATIC_DIR` pointed at `frontend/dist` -- no Vite, no proxy, no second port. A fallback that runs a *different* stack is a second thing to debug on stage.
- **It refuses to start quietly.** Everything that could be wrong is checked before the browser opens: the virtualenv exists, the build exists and is not older than `src/` (it rebuilds if it is), the API answers, both sites report, **one verifies and one needs review** -- the contrast the whole pitch turns on -- the exported report id resolves, and a mistyped API path still 404s as API rather than as the page shell. `npm run demo:check` runs the lot and exits.
- **It proves it needs no internet.** The built shell and every asset are scanned for external origins, and the check fails if one reappears. This is only true because Phase 6.5 dropped the Google Fonts link when the type moved to Times New Roman -- before that, the "offline" fallback would have silently degraded its own typeface on a dead network.
- **A busy port is handled rather than fatal.** `npm run dev` left running would otherwise make the fallback fail exactly when it is needed; it takes the next free port instead and prints the URL it actually bound. Safe here specifically because this process serves the page *and* the API, so there is no proxy that could point at a stale backend -- the Phase 4 failure that made the dev launcher strict in the first place.
- **Proven non-vacuous with two injected regressions:** a Google Fonts link put back into `dist/index.html` (caught, named the URL) and port 8000 held by another uvicorn (moved to 8001, printed why). Both clean on revert.
