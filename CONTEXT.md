...
- **Dark is the default**: a deep ocean ground (`#050e13`) with a slow three-wash aurora behind everything, glass panels (translucent `--surface` + `backdrop-filter`) that are translucent *to* that aurora, and teal that actually glows. Algae is the one subject where bioluminescence is not a gimmick.
