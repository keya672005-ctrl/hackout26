...
The **palette is the Phase 0 design canvas** -- teal `#0f766e` accent, amber for "needs review", slate ink on white cards, `#f8fafc` page. An editorial moss/bone repaint was tried after the mid-eval and **rejected on sight**; the palette was reverted and is not up for relitigation. What was kept from that pass is the **typography and composition**, which are now the interface's identity:

- **One typeface throughout: Times New Roman.** No sans anywhere. Hierarchy is carried by size, weight, letter-spacing and whitespace instead of by mixing families. Every field name, unit, badge, axis caption and control is set as a small uppercase technical label (`letter-spacing: .14em`); figure titles are large serif; body copy is 17px/1.5 because Times sets small. `src/index.css` keeps `--font-heading` / `--font-body` / `--font-mono` as names, all pointing at the one serif stack -- which includes Liberation Serif and Nimbus Roman, the metric-compatible faces that exist on Linux, because the Docker image has no Times New Roman.
- **Exactly one teal-gradient panel per screen**, and no more: the masthead on the overview, the headline CO2 tile on the site screen, the certified-figure card on the report. All three revert to ink on white in print, because a printer that drops backgrounds would otherwise drop their white text with them.
- Charts keep the canvas series colours (sensor `#2a78d6`, satellite `#eb6834`) -- validated for colour-blind separation, and deliberately neither the brand teal nor the status amber.
