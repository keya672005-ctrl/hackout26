...
The interface is **art-directed as a scientific journal**, not as a SaaS dashboard: Aesop x research publication. It was restyled away from the original Phase 0 canvas (white cards, teal `#0f766e`, Sora / IBM Plex) after the mid-eval -- the layout and the three screens are unchanged, only type and colour.

- **One typeface throughout: Times New Roman.** No sans anywhere. Hierarchy is carried by size, weight, letter-spacing and whitespace instead of by mixing families. Every field name, unit, badge, axis caption and control is set as a small uppercase technical label (`letter-spacing: .14em`); figure titles are large serif; body copy is set at 17px/1.5 because Times sets small. `src/index.css` keeps `--font-heading` / `--font-body` / `--font-mono` as names, all pointing at the one serif stack -- which includes Liberation Serif and Nimbus Roman, the metric-compatible faces that exist on Linux, because the Docker image has no Times New Roman.
- **Moss + bone + ink**, roughly 70% warm neutral / 20% ink + deep moss / 8% moss / 2% light moss. Canvas `#EEEDE4`, surface `#F8F7EF`, deep moss `#34483A`, moss `#687B55`, light moss `#AAB394`, ink `#252B26`. Never pure white, never pure black **on screen** -- paper is the deliberate exception (see the print block in `index.css`). Charts are one monochromatic green system: sensor `#687B55` solid, satellite `#34483A` dashed.
- **Exactly one moss-gradient panel per screen**, and no more: the masthead on the overview, the headline CO2 tile on the site screen, the certified-figure card on the report. All three revert to white-on-ink in print, because a printer that drops backgrounds would otherwise drop bone-coloured text with them.
- **The logo is the argument.** One algae leaf: the solid half is the sensor record, the dashed half the independent satellite proxy drawn against it -- they only close into a leaf when the two agree. `frontend/public/favicon.svg` is both the favicon and the header mark.
...
- [~] **Phase 6 — Testing & Deployment** — the app now deploys as **one service on one origin**: FastAPI serves the built SPA itself (`Dockerfile` + `render.yaml`, Render free plan, health check on `/health`). 120/120 checks pass via `node verify_phase6.mjs` in `frontend/`, and 116/116 against the **Docker image itself** via `node verify_phase6.mjs --target http://127.0.0.1:8123`. Live at **https://algae-carbon-platform.onrender.com** — **117/117** against the deployed service via `node verify_phase6.mjs --target https://algae-carbon-platform.onrender.com`. **Remaining:** open the link on a real phone and tick that item by hand (see `prd.md` §11 Phase 6)
- [ ] Phase 7 — Pitch Prep
